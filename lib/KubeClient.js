...
var KubeClient = {
	...
	"update": function(object) {
		...
		if (typeEndpoints[object.kind]) {
			path = "namespaces/" + namespace + "/" + typeEndpoints[object.kind].path + '/' + object.metadata.name
  ...
  "create": function(object) {
		...
		if (typeEndpoints[object.kind]) {
			path = "namespaces/" + namespace +"/" + typeEndpoints[object.kind].path
			prefix = typeEndpoints[object.kind].prefix
		} else {
			throw new Error("Cannot identify Kubernetes object type of an object when about to post to the API.")
		}
	...
	"deleteDeployment": function(id) {
		return this.delete('namespaces/' + namespace + '/deployments/' + id, "v1beta1")
	},

	"deleteService": function(id) {
		return this.delete('namespaces/' + namespace + '/services/' + id, "v1")
	}
