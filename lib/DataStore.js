...
var DataStore = {
  "getApplications": function() {
    return pool.getConnection()
      .then((conn) => {
         var res = conn.query('SELECT * FROM applications WHERE active = 1');
         conn.release();
         return res;
      }).then( (result) => {
  ...
  "updateDeployment": function(deploymentConfig) {
    var x = deploymentConfig.toDB()
    console.log("About to update", x.size, x.id)
    return simpleQuery("UPDATE deployments SET meta = ?, size = ? WHERE id = ?", [x.meta, x.size, x.id]);
  },
  ...
  "getAllDNSentries": function() {
    return simpleQuery("SELECT * FROM service_dns", []);
  },

  "getAllDNSentriesFromInfrastructure": function(infrastructure) {
    return simpleQuery("SELECT * FROM service_dns JOIN deployments ON (service_dns.`deployment` = deployments.`id`) WHERE deployments.`infrastructure` = ?", [infrastructure])
  }
